...
class AuthController extends BaseController {
    ...
    public function __construct() {
        ...
        $this->_private_key = $this->_p_16->toString();//私钥
        $m="";
        $p = "";
...
//        判斷有沒有#号
        $pos = strpos($data,"#");
        if ($pos === false) {
//            没有直接用ras解密，hex2bin函数是把十六进制值转换为 ASCII 字符：就是转成010101类似的数据
            $d = rsa_decrypt(hex2bin($data), $this->_private_key, $this->_mend, $this->_keylength);//四个参数，密文，私钥，公钥，密匙长度
            // 反向
            $unsecret = strrev($d);//明文
